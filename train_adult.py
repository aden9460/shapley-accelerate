...
def load_raw_data(path):
    ...
    categorical_attribs = ["workclass", "education", "marital-status", "occupation", "relationship", "race", "native-country", "sex"] #分类属性，共8种
    ...
    real_value = StandardScaler().fit_transform(input_data.loc[:, real_attribs].values)  # 真实值（32561,5），标准化数据
    ...
    cate_attrib_book_buf = []#储存

    for cate_attrib_index, cate_attrib in enumerate(categorical_attribs):

        enc = OneHotEncoder() # handle_unknown='ignore')
        enc.fit(input_data.loc[:, cate_attrib].values.reshape(-1,1))#挑选出来当前列的数据，形状是(32561,1)
        cate_attrib_encoded = enc.transform(input_data.loc[:, cate_attrib].values.reshape(-1,1)).toarray()  #用onehot编码分类属性，形状是(32561,9)
        cate_attrib_book = np.eye(cate_attrib_encoded.shape[-1]) # np.unique(cate_attrib_encoded, axis=0)   #生成9*9的单位矩阵
        cate_attrib_encoded_buf.append(cate_attrib_encoded)#将onehot编码后的数据存储
        scaler = StandardScaler()#标准化类，均值为0，标准差为1
        scaler.fit(cate_attrib_encoded)#    
        cate_attrib_scaled = scaler.transform(cate_attrib_encoded)#把onehot编码后的数据标准化
        cate_attrib_book_scaled = scaler.transform(cate_attrib_book)#将单位矩阵标准化
        reference = cate_attrib_encoded.mean(axis=0).reshape(1,-1)#求出onehot编码后的数据的均值，形状为（1,9）
        cate_attrib_book_scaled = np.concatenate((cate_attrib_book_scaled, reference), axis=0) # -1 for background noise #生成9*10的矩阵，包含了均值独热码和数据均值
        # cate_attrib_book_scaled = np.concatenate((reference, cate_attrib_book_scaled), axis=0) # 0 for background noise

        cate_attrib_raw = cate_attrib_encoded.argmax(axis=1).reshape(-1,1) # -1 for background noise #生成onehot编码后的数据的最大值
        # cate_attrib_raw = cate_attrib_encoded.argmax(axis=1).reshape(-1,1) + 1 # 0 for background noise
        cate_attrib_encoded_buf.append(cate_attrib_scaled)#将标准化后的数据存储
        cate_attrib_raw_buf.append(cate_attrib_raw)#将最大onehot编码保存
        cate_attrib_index_buf.append(real_value.shape[-1] + cate_attrib_index)#将分类属性的下标存储
        cate_attrib_book_buf.append(cate_attrib_book_scaled)#将onehot编码矩阵存储，保存每个分类特征的独热编码参考矩阵
    ...
    cate_attrib_index_buf = np.array(cate_attrib_index_buf, dtype=np.int64)#将分类属性的下标转化为int

    # print(real_value.values)
    # print(cate_attrib_encoded_buf)

    X_raw = np.concatenate([real_value] + cate_attrib_raw_buf, axis=1)#保存onehot编码后的数据
    # X = pd.concat([real_value, categorical_value], axis=1)
    X = np.concatenate([real_value] + cate_attrib_encoded_buf, axis=1)#同时保存分类属性的onehot编码和标准化后的数据，其中cate_attrib_encoded_buf是onehot编码后的数据，
    #包含了每类的onehot编码和标准化后的数据，所以X的形状是（32561,209）
    ...
    return X, y.values, Z.values, X_raw, cate_attrib_book_buf, real_attrib_index_buf, cate_attrib_index_buf#分别返回了混合属性编码，性别，收入，onehot最大编码，每个分类特征的独热编码参考矩阵
...
def load_data(path, val_size, test_size, run_num=0):
    ...
    print("Majority num:", Z[Z==1].shape[0])#是大于50k收入群体的数量
    print("Minority num:", Z[Z==0].shape[0])
    print("Positive num:", Y[Y==1].shape[0])#男性数量
    print("Negative num:", Y[Y==0].shape[0])



    x_train_all, x_test, y_train_all, y_test, z_train_all, z_test = train_test_split(X, Y, X_raw, test_size=test_size, random_state=run_num)#分别将混合属性编码，性别，onehot最大编码分割成训练集和测试集
    x_train, x_val, y_train, y_val, z_train, z_val = train_test_split(x_train_all, y_train_all, z_train_all, test_size=val_size, random_state=0) # , random_state=fold)#继续分割为验证集和训练集
    ...
    cate_attrib_book = [torch.from_numpy(x).type(torch.float) for x in cate_attrib_book]#将onehot编码矩阵转化为tensor
...
if __name__ == "__main__":
    ...
    round_num = 0
    datasets_torch, cate_attrib_book, dense_feat_index, sparse_feat_index = load_data('/opt/data/private/wzf/u_shapley/shapley_accelerated/adult-dataset/adult.csv', val_size=0.2, test_size=0.2, run_num=round_num) #
