...
class Shapley_accelereated():
    ...
    def get_second_order_grad(self, layer):

        with torch.set_grad_enabled(True):
            # hook = layer.register_forward_hook(self.hook_fn)
            model.train()
            for x, labels in self.data_generator:
                y = self.model(x)
                ...
                target_neuron.requires_grad = True
                self.target_module = target_neuron
                final_matrix = []
                # for weight in target_neuron:
                #     grads = autograd.grad(loss, weight, create_graph=True, retain_graph=True,allow_unused=True)

                grad_list = []
                grads = autograd.grad(loss, target_neuron, create_graph=True)[0]
                for grad in grads:
                    grad2 = autograd.grad(grad.sum(), target_neuron, create_graph=True, retain_graph=True, allow_unused=True)[0]
                    grad_list.append(grad2)
                grad_matrix = torch.stack(grad_list)
                final_matrix.append(grad_matrix)
                self.reference = torch.mean(target_neuron, dim=1)
                    # hook.remove()
                return torch.abs(final_matrix) 
