...
from torchpruner.attributions.methods.shapley_values import ShapleyAttributionMetric


class Efficient_shap_dnn(ShapleyAttributionMetric):
    def __init__(self, f, reference, topK,target_module, whole_layer,data_gen,noise=0):
        self.f = f
        # self.topK = min(topK, reference.shape[-1]-1)
        self.topK = min(topK, target_module.shape[-1]-1)
        
        self.model = f
        # self.M = reference.shape[-1]
        self.M = target_module.shape[-1]
        self.M_inter = topK + 1
    ...
    def feature_selection(self, error_matrix):
        ...
        for index in range(self.M):
            ...
            allinter = np.sort(np.array(list(allinter)).astype(np.int64))

            local_idx = np.where(allinter == index)[0]  # .squeeze(axis=0)
            inter_index = np.zeros(self.M).astype(np.bool_)
            inter_index[allinter] = True
            other_index = np.ones(self.M).astype(np.bool_)
    ...
    def brute_force_forward(self):
        ...
        set0 = torch.cat((self.mask, torch.zeros((self.mask.shape[0], 1)).byte()), dim=1)#(2048,12)
        set1 = torch.cat((self.mask, torch.ones((self.mask.shape[0], 1)).byte()), dim=1)#(2048,12)
    ...
    def f_mask(self, x_inter):
        ...
        # print(x_mask)
        # print(self.f(x_mask))
        self.mask_index = x_mask  
        return self.run(self.whole_layer,sv_samples=1)
    ...
    def _forward_hook(self):
        def _hook(module, _, output):
            ...
            return output.index_fill_(
                self.mask_index, torch.tensor(self.mask_indices).long().to(self.device), 0.0,
            )
    ...
    def run_module(self, module, samples):
        ...
        with torch.no_grad():
            ...
            n = len(self.mask_index)  # output dimension
            ...
            for j in range(samples):
                ...
                for i in np.random.permutation(n):
                    self.mask_indices = self.mask_index
                    new_loss = self.run_all_forward()
                    sv[:, i] += ((new_loss) / samples).squeeze().detach().cpu().numpy()
                    # loss = new_loss
